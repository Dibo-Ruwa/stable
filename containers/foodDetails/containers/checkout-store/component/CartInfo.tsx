...
export const CartInfo: React.FC<CartDropdownProps> = ({ subtotal, deliveryFee, total }) => {
  ...
  const incrementExtra = (itemId: string, extraId: string) => {
    ...
    if (item) {
      const extra = item.extras?.find((extra) => extra._id === extraId);
      if (extra) {
        const newQuantity = (extra.quantity || 0) + 1;
        updateExtraQuantity(itemId, extraId, newQuantity);
      }
    }
  };

  // Handle decrement for extras
  const decrementExtra = (itemId: string, extraId: string) => {
    const item = cartItems.find((item) => item._id === itemId);
    if (item) {
      const extra = item.extras?.find((extra) => extra._id === extraId);
