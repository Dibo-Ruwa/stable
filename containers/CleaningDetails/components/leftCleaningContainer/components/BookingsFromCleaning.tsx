...
export const BookingsFromCleaning = () => {
  ...
  // Filter quotes to show only cleaning type requests
  const cleaningRequests = quotes?.filter(quote => quote.type === 'cleaning') || [];
  ...
  return (
    <Container>
      <Title>Current Cleaning Bookings</Title>
      <RequestsList>
        {cleaningRequests?.map((request) => (
          <RequestCard key={request._id}>
            <RequestHeader>
              <ServiceType>{request?.type === 'cleaning' ? 'Cleaning / Service' : ''}</ServiceType>
